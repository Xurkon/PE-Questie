...
-- 3.3.5 compatibility: fake Item API if missing (Retail style)
if not Item then
    Item = {}
end
...
if not Item.CreateFromItemID then
    function Item:CreateFromItemID(itemID)
        ...
        function obj:ContinueOnItemLoad(callback)
            -- In 3.3.5 there is no async loading, so call the callback immediately
            if callback then
                callback()
            end
        end
...
function QuestieDB.IsComplete(questId)
    ...
    if not questLogEntry then 
        if questId == 12843 or questId == 12844 then print("QuestieDB IsComplete " .. tostring(questId) .. " returning 0 due to no questLogEntry") end
        return 0 
    end
    if questLogEntry.isComplete == 1 or questLogEntry.isComplete == -1 then
        if questId == 12843 or questId == 12844 then print("QuestieDB IsComplete " .. tostring(questId) .. " returning " .. tostring(questLogEntry.isComplete) .. " due to explicit isComplete flag") end
        return questLogEntry.isComplete
    end

    local objectives = questLogEntry.objectives
    if not objectives or #objectives == 0 then
        local ret = (noQuestItem and 0) or 1
        if questId == 12843 or questId == 12844 then print("QuestieDB IsComplete " .. tostring(questId) .. " returning " .. tostring(ret) .. " due to no objectives") end
        return ret
    end
    ...
    for _, obj in ipairs(objectives) do
        if obj.numRequired and obj.numRequired > 0 and obj.numFulfilled ~= obj.numRequired then
            if questId == 12843 or questId == 12844 then
    ...
    if (questId == 12843 or questId == 12844) and allDone then
         print("QuestieDB " .. tostring(questLogEntry.title) .. " thinks it is all done based on objectives!")
    end
...
-- =========================
-- Ascension custom DB merge
-- =========================
local function _Asc_LoadIfString(data, label)
    if not data then return nil end
    if type(data) == "string" then
        local fn, err = loadstring(data)
        if not fn then
            if Questie and Questie.Debug then
                Questie:Debug(Questie.DEBUG_CRITICAL, "[AscensionDB] loadstring failed for " .. tostring(label) .. ": " .. tostring(err))
            end
            return nil
        end
        ...
        if not ok then
            if Questie and Questie.Debug then
                Questie:Debug(Questie.DEBUG_CRITICAL, "[AscensionDB] executing chunk failed for " .. tostring(label) .. ": " .. tostring(tbl))
            end
            return nil
        end
        return tbl
...
function QuestieDB:LoadAscensionQuestData()
    local A = QuestieLoader:ImportModule("AscensionDB")
